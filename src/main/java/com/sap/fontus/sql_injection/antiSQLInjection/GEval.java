...
class evalVisitor implements IExpressionVisitor {
	...
	public boolean exprVisit(TParseTreeNode pNode, boolean isLeafNode) {
		...
		case simple_source_token_t:
			if (expr.getSourcetokenOperand().toString()
		...
		case unary_minus_t:
			try {
				Long l = Coercion.coerceLong(((TExpression) exprs.pop())
						.getVal());
				expr.setVal(-l);
			} catch (Exception e) {
				e.printStackTrace(); // To change body of catch statement use
	...
	Object eval_constant(TExpression expr) throws Exception {
		...
		if (constant.getNodeType() == ENodeType.T_Constant_Integer.getId()) {
			Integer v = Coercion.coerceInteger(constant.getStartToken()
					.toString());
			...
			Double d = Coercion.coerceDouble(constant.getStartToken()
					.toString());
			ret = d;
		} else if (constant.getNodeType() == ENodeType.T_Constant_String
				.getId()) {
			String s = constant
					.getStartToken()
					.toString()
	...
	Object eval_like(TExpression expr) throws Exception {
		...
		if ((left instanceof UnknownValue) || (right instanceof UnknownValue)) {
			return new UnknownValue();
		}
		...
		if (right.toString().startsWith("%")) {
			if (right.toString().endsWith("%")) {
				...
				if (left.toString().contains(c)) {
					if (expr.getOperatorToken().toString()
			...
			if (left.toString().startsWith(c)) {
				if (expr.getOperatorToken().toString().equalsIgnoreCase("not")) {
					return Boolean.FALSE;
				} else {
					return Boolean.TRUE;
				}
	...
	Object eval_notequal(TExpression expr) throws Exception {
		...
		if (left == null && right == null) {
			...
			return (Coercion.coerceDouble(left).equals(Coercion
					.coerceDouble(right))) ? Boolean.FALSE : Boolean.TRUE;
		} else if (left instanceof Number || right instanceof Number
				|| left instanceof Character || right instanceof Character) {
			return (Coercion.coerceLong(left)
					.equals(Coercion.coerceLong(right))) ? Boolean.FALSE
					: Boolean.TRUE;
		} else if (left instanceof Boolean || right instanceof Boolean) {
			return (Coercion.coerceBoolean(left).equals(Coercion
					.coerceBoolean(right))) ? Boolean.FALSE : Boolean.TRUE;
	...
	Object eval_group_comparison_conditions(TExpression expr) throws Exception {
		Object right = ((TExpression) exprs.pop()).getVal();
		Object left = ((TExpression) exprs.pop()).getVal();
	...
	Object eval_simple_comparison_conditions(TExpression expr) throws Exception {
		Object right = ((TExpression) exprs.peek()).getVal();
		Object left = ((TExpression) exprs.peek()).getVal();
	...
	Object eval_between(TExpression expr) throws Exception {
		...
		if ((between instanceof UnknownValue) || (left instanceof UnknownValue)
			...
			return (betweenValue >= leftValue && betweenValue <= rightValue) ? Boolean.TRUE
					: Boolean.FALSE;
		}
	...
	Object eval_mod(TExpression expr) throws Exception {
		...
		if (left instanceof Float
				...
				|| (left instanceof String && (((String) left).indexOf(".") != -1
						|| ((String) left).indexOf("e") != -1 || ((String) left)
						.indexOf("E") != -1))
				|| (right instanceof String && (((String) right).indexOf(".") != -1
						|| ((String) right).indexOf("e") != -1 || ((String) right)
						.indexOf("E") != -1))) {
			Double l = Coercion.coerceDouble(left);
			Double r = Coercion.coerceDouble(right);
