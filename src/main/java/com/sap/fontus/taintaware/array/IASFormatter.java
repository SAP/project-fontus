...
import com.sap.fontus.taintaware.shared.IASAbstractStringBuilderable;
import com.sap.fontus.taintaware.shared.IASFormatterable;
    ...
    private static class Transformer {
        ...
        private IASString transformFromHashCode() {
            ...
            if (null == arg) {
                ...
                if (arg instanceof IASArrayAware && ((IASTaintAware) arg).isTainted()) {
                    int[] taints = ((IASArrayAware) arg).getTaints();
                    int taint = 0;
                    for (int t : taints) {
                        if (t != 0) {
                            taint = t;
                    ...
                    taintedHexString.getTaintInformation().setTaint(0, taintedHexString.length(), taint);
                }
        ...
        private IASString padding(IASAbstractStringBuilderable source, int startIndex) {
            ...
            if (length >= width) {
                return (IASString) source.toIASString();
            }
