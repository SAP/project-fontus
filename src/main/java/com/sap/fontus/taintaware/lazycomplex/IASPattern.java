...
import com.sap.fontus.taintaware.lazycomplex.operations.SplitOperation;
...
public final class IASPattern implements IASPatternable {
    ...
    public IASString[] split(CharSequence input, int limit) {
        ...
        for (int i = 0; i < splitted.length; i++) {
            strings[i] = inputString.derive(splitted[i], new SplitOperation(this.patternString.getString(), i, limit), false);
        }
        return strings;
    }
